...
#    vols1.groupby(['RecordNumber', 'TakeType', 'Wap'])['GwAllocationBlock'].count()
    ...
    vols1['FullAnnualVolume'] = (vols1['FullAnnualVolume'] * vols1['ratio']).round()  ##-WILCO COMMENT: Maybe remove the round() here if the output is used as input in another function to prevent aggregating rounding errors
